...
app.get('/', (req, res) => {
  ...
  app.get('/validarfecha/:ano/:mes/:dia', (req, res) =>{
        let ano = ValidacionesHelper.getIntegerOrDefault(req.params.ano,' ');
        let mes = ValidacionesHelper.getIntegerOrDefault(req.params.mes,' ');
        let dia = ValidacionesHelper.getIntegerOrDefault(req.params.dia,' ');
       
        
        if (DateTimeHelper.isDate(ano,mes,dia) == false)
        {
  ...
  {
    let numero1 = ValidacionesHelper.getIntegerOrDefault(req.params.n1,' ');
    let numero2 = ValidacionesHelper.getIntegerOrDefault(req.params.n2,' ');
  ...
  {
    let txt = ValidacionesHelper.getStringOrDefault(req.params.texto," ");
    let p = ValidacionesHelper.getIntegerOrDefault(req.params.p, " ");
    let response = "";
  ...
  {
    let id = ValidacionesHelper.getStringOrDefault(req.params.imdbID, " ");
    let response= "";
  ...
  {
    let dniIngreso = req.body.dni;
    let validar = false;
    for (let i = 0; i < alumnosArray; i++) {
      if(alumnosArray[i].dni == dniIngreso){
  ...
  app.get("/fechas/isDate?fecha=2024-04-12", (req,res) =>
  {
      let f = req.query.fecha;
      let n  = f.split("-");
      if(DateTimeHelper.isDate(n[0],n[1],n[2]) == true)
      {
        res.status(200).send("OK");
      }
      else
  ...
  app.get("/fechas/getEdadActual?fechaNacimiento=1972-07-13", (req,res) =>
  {
    let B = req.query.fechaNacimiento;
  ...
  app.get("/fechas/getDiasHastaMiCumple?fechaNacimiento=1972-07-13", (req,res) =>
  {
      let n = req.query.fechaNacimiento;
      let b = n.split("-");
